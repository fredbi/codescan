...
- ✅ **Save / reload file** (`32876f6`) — `F5` re-reads the open file, asking only
  when the buffer is dirty. `y` discards, `n`/`esc`/**`enter`** decline: the
  destructive answer has to be typed on purpose. Bound inside the editor too,
  that being the state the guard exists for — a textarea otherwise swallows it.
  `ctrl+r` deliberately NOT also bound (redo / reverse-search in too many
  editors). Reload restores the line by number and lands in the read-only viewer.
- ✅ `?` help overlay (3f872dd) — plus an `h: help` chip in the header, since a
  help key nobody can see is not a help key.
- ✅ **Adjustable split sizes** (`13397fb`) — `ctrl+←/→` and `ctrl+↑/↓`, each
  divider travelling in its arrow's direction. Held as PROPORTIONS, so a terminal
  resize keeps the chosen layout. Bounded short of either edge for
  recoverability: a collapsed pane cannot be dragged back, since the keys that
  would restore it live in a status line it no longer has room for.
- ⛔ **Light/dark theme** — deferred 2026-08-07, not dropped. Two findings stand
  for whenever it returns: `theme.go` states its constants-not-variables choice
  deliberately (so a switch wants a `Palette` value fixed once at startup, not a
  mutable global), and `Match`/`Selected`/`Follower`/`Chip`/`Stale` hardcode
  colours outside the palette and must move in first or the light theme lands
  half-applied. `termenv` is already a direct dependency, so background detection
  is free; the real cost is CHOOSING the palette.

### Landed beyond this backlog (2026-08-07, `tui-ux-enhancements`)

Full account in `tui-ux-enhancements.md`. Beyond the two items above:
severity-coloured diagnostic rows, diagnostic → spec tracking, a `v`
spec-validation tab, and an annotation reference on `K` / click. Three rendering
bugs were found and fixed on the way — scrolling modals resizing themselves, the
header running off a narrow terminal, and the keymap silently outgrowing a
screen.
